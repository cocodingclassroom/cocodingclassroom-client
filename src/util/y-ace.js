...
export class AceCursors{
	...
	constructor(binding){
		...
		this.marker.update = function(html, markerLayer, session, config) {
			...
			let cursorsClear = this.self.ace.container.querySelectorAll('.cursor').forEach((elm)=>{elm.remove()})
			...
			for (let i = 0; i < cursors.length; i++) {
				...
				if (cursor.row < start) {
					...
					let screenPos = session.documentToScreenPosition(cursor.row, cursor.col)
					...
					let left = markerLayer.$padding + aceGutter + (screenPos.column * width) + 0
					...
					// draw cursor and flag
						...
						el.style = `
							...
							cursor: help;
						`
						el.innerHTML = '<div class="cursor-label" style="background: '+client.user.color+';top: -1.3em;white-space: nowrap;">'+client.user.name+'</div>'
		...
		this._awarenessChange = ({ added, removed, updated }) => {
				...
				updated.forEach(id => {
					...
					if(client.hasOwnProperty('cursor') && client.cursor.ace == this.aceID){
						cursorsCurrent.push(this.parseCursor(id))
					}
				})

				// only redraw if changes detected
				if(JSON.stringify(cursorsCurrent) !== JSON.stringify(this.cursorsMemory)){
		...
		this._cursorObserver = () => {
				...
				let cursor = {id:this.doc.clientID, ace:this.aceID, sel:!curSel.isEmpty()}

				let indexAnchor = aceSession.doc.positionToIndex(curSel.getSelectionAnchor())
				...
				if (client[this.awarenessCursor] == undefined || cursor.anchor !== client[this.awarenessCursor].anchor || cursor.head !== client[this.awarenessCursor].head || cursor.ace !== client[this.awarenessCursor].ace) {
		...
		if(client !== undefined && client.hasOwnProperty(this.awarenessCursor)){
			...
			// }
			 ...
			 if(c.sel){
				...
				if(customStyle){
					customStyle.innerHTML = '.selection-' + c.id + ' { position: absolute; z-index: 20; opacity: 0.5; background: '+client.user.color+'; }'
				}else{
	...
	destroy () {
		...
		this.aceSession.selection.off('changeSelection', ()=>this._cursorObserver())
	}
}
