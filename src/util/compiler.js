...
export const interpret = (
    ...
    if (!fullRebuild && !activeError) {
        let softInterpretResultFromBinding = activeBinding.bindingSpecificSoftCompile(editor)
...
const fullRebuildOfIframe = (
    ...
    if (!sketchCode.match(/(\/\/).*(no.*protect)/g)) {
        try {
            ...
            // validCode = false;
            errorCallback(`❌ error found near line ~ ${e.lineNumber}, ${e.description}`)
    ...
    iframe.sandbox = 'allow-same-origin allow-scripts allow-downloads allow-pointer-lock'
...
const softInterpretation = (codeType, codeName, codeStart, codeEnd, editor, iframe, errorCallback) => {
    let curCodeTemp = ''
