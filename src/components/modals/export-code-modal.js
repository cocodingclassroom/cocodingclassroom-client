        ...
        () => {
            ...
            switch (exportType) {
                case 'export-room-js':
                    ...
                    BindingService.get().binding.exportRoomJS(RoomService.get().getRoom(roomId))
                    BindingService.get().binding.exportPicture(RoomService.get().getRoom(roomId))
                    break
                case 'export-room-html':
                    console.log(exportType)
                    room.l_filename = filename
                    BindingService.get().binding.exportRoomHTML(RoomService.get().getRoom(roomId))
                    break
