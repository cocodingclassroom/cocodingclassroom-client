...
export class MenuView extends LitElement {
    ...
    render = () => {
        ...
        return html`
            <div class="cc-meta">
                ${room.isTeacherRoom()
                    ? html` <cc-teacher-menu-view roomId="${room.id}"></cc-teacher-menu-view>`
                    : html` <cc-student-menu-view roomId="${room.id}"></cc-student-menu-view>`}
                <cc-user-list roomId="${room.id}"></cc-user-list>
                ${this.#renderChat()}
            </div>
        `
    ...
    #renderChat = () => {
        return html` <div class="border-left-right background-dark">
            ...
            <div id="message_container" class="messages">${this.#renderMessages()}</div>
    ...
    #renderTrash = () => {
        let room = RoomService.get().getRoom(this.roomId)
        if (room.isTeacherRoom() && UserService.get().localUser.isStudent()) return html``

        return html` <div data-tip="Clear Chat" class="pointer pulse" @click="${() => clearChat(this.roomId)}">
