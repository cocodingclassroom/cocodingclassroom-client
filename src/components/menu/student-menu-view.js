...
import { basicFlexStyles, menuRowStyles, pulseStyle, toolTipStyle } from '../../util/shared-css'
...
export class StudentMenuView extends LitElement {
    ...
    #renderRoomClaim = () => {
        ...
        if (room.isClaimed()) {
            if (room.isOwnedByLocalUser() || UserService.get().localUser.isTeacher()) {
    ...
    #renderOwnedButNotUnlockableByThisUser() {
        return html`
            <div
                class="disabled"
                data-tip="${RoomService.get().getRoom(this.roomId).getOwnerAsUser().getNameShortened()} locked room"
            >
                <cc-icon svg="${iconSvg.lock}"></cc-icon>
            </div>
        `
