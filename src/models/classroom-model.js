...
export class ClassroomModel extends YSyncModel {
    ...
    constructor(classRoomId, liveCoding, liveCodingDelay, lineNumbers, roomLocks, walkDelay, bindingIndex) {
    ...
    addUser(user) {
        ...
        if (!this.peopleIds.toArray().includes(user.id)) this.peopleIds.push([user.id])
    }
